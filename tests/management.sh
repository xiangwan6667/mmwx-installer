...
mkdir -p "$ROOT/data"
printf current-data > "$ROOT/data/value"
printf '{"version":"v6","app":"app@sha256:old","channel":"beta"}' > "$ROOT/image-rollback.json"
...
[[ $(cat "$ROOT/data/value") == current-data && ! -f $ROOT/image-rollback.json ]]
[[ $(jq -r .version "$ROOT/state.json") == v6 ]]
